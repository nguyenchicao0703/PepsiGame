...
const ItemGiftExchange = (props) => {
    ...
    useEffect(() => {
        ...
        database().ref(`users/${mobile}/collection/score`).on('value', snapshot => {
            setTotalScore(snapshot.val());
        });

        // Get remaining product quantity and save to state remaining
        database().ref(`products/${data.id}/remaining`).on('value', snapshot => {
            setRemaining(snapshot.val());
        });

        // Get current value of quantity
        database().ref(`users/${mobile}/reward/${data.id}/quantity`).once('value', snapshot => {
